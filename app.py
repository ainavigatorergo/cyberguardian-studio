...
st.set_page_config(page_title="CyberGuardian Studio", layout="wide")
st.title("🔥 CyberGuardian Studio — AI Video Studio")

# Выбор канала
channel = st.selectbox(
    "📢 Канал",
    ["CyberGuardianSec (кибербезопасность)", "AI Navigator (нейросети)"]
)

# Ввод темы
topic = st.text_input(
    "📝 Тема видео",
    placeholder="Например: Как защитить аккаунт от взлома"
)
...
with col1:
    video_format = st.radio(
        "Формат видео",
        ["Полное (16:9)", "Короткое (9:16 для Shorts/TikTok)"],
        index=0
    )
with col2:
...
if st.button("🚀 Создать видео") and topic:
    with st.spinner("🔍 Исследование..."):
        ...
        st.text_area("📊 Исследование", research, height=150)
